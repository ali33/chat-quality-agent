...
- **`Dockerfile`**: build backend với **CGO** (`gcc` + `musl-dev` trên Alpine) vì driver SQLite `mattn/go-sqlite3`.
...
- **`build-backend.bat`**: `go mod tidy` + `go build` → `build/cqa-server.exe` (`CGO_ENABLED=1`).
...
| Build Windows | (không có sẵn) | `build-frontend.bat` + `build-backend.bat` |
